...
WEBSITE = "https://quintessential-quintuplets.com/"
REGEX_CHAPTERS = r"<li><a href=\"(.*?)\">The Quintessential Quintuplets, Vol\.([\d]+) Chapter ([\d.]+)(?: )?(.*?)?<\/a><\/li>"
...
for match in re.finditer(REGEX_CHAPTERS, requests.get(WEBSITE).text, re.MULTILINE):
    ...
    chapter_name = match.group(4).replace("&#8217;", "’").replace("&#8211;", "–").replace(":", " -")
    ...
    chapter_beautiful_name = ("Vol. {} Ch. {}" + (" - {}" if chapter_name != "" else "")).format(volume, chapter, chapter_name)
